...
if (room == rm_game_mp or room == rm_game_sp) and obj_deck.game_over {
	if mouse_check_button_pressed(mb_any) or keyboard_check_pressed(vk_anykey) with obj_game_exit event_perform(ev_keypress,vk_escape);
}
	...
	else if !instance_exists(obj_pointer) and (keyboard_check_pressed(vk_anykey) or global.pointeron) {
		create_pointer();
		global.pointeron = true
	}
	
	//moves the pointer 
	else if not (keyboard_check(vk_enter) or keyboard_check(vk_space)) and (press_left() or press_right() or press_up() or press_down()) pointer_controls()	
