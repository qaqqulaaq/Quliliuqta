...
function draw_card(_num){
	...
	var _pos = card_place(instance_number(obj_parcard))
	var _x = 570*global.winscale
	var _y = 240*global.winscale
	
	instance_create_layer(_x,_y,"instances",obj_parcard,{val: _num, targ_x: _pos[0], targ_y: _pos[1], hspeed: (_pos[0]- _x)*2 / game_get_speed(gamespeed_fps), vspeed: (_pos[1] - _y)*2 / game_get_speed(gamespeed_fps)});
